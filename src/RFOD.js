...
ipcRenderer.on('RFODResult', (event, result) => {
    if(result) {
...
class RFOD extends Component {
    ...
    handleSubmit(evt) {
        evt.preventDefault();
        ipcRenderer.send('RFOD', this.state);
        ...
        return (
            <div className="App">
                ...
                <div style={{color:'white', fontSize: 20, marginLeft:80, marginBottom:40, marginTop:13}}>
                    <b><u>RECEIVE FROM OTHER DISP.</u></b>
                </div>
                ...
                <div className="labelClass">
                    <label htmlFor="date">Dispensary</label>
                </div>
                <input 
                    type="text" 
                    className="inputText" 
